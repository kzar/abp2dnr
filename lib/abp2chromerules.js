...
function generateAllowRules(filter, urlFilter, matchCase)
{
  ...
  if (contentType & contentTypes.DOCUMENT)
  {
    ...
    if (contentType & contentTypes.SUBDOCUMENT)
    {
 ...
 * @param {function} [checkValidRE2]
 *   For regular expression filter support, provide this function which returns
 *   true if the given regular expression is supported RE2 syntax and false
 *   otherwise. If omitted, regular expression filters will be ignored.
 *   See https://github.com/google/re2/wiki/Syntax
 */
function ChromeRules(firstId = 1, checkValidRE2 = null)
{
  this.nextId = firstId;
  this.checkValidRE2 = checkValidRE2;
    ...
    if (urlFilter)
    {
      ...
      if (urlFilter.startsWith("||*"))
        urlFilter = urlFilter.substr(3);
             ...
             !(this.checkValidRE2 && this.checkValidRE2(filter.regexp.source)))
    {
      return false;
    }
