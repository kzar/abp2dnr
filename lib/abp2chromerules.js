...
function generateRule(filter, id, genericBlockExceptionDomains)
{
  ...
  // doing that if the filter only contains the hostname part, or if the
  // path part doesn't contain any letters.
  if (!filter.matchCase && !filter.canMatchAsLowercase)
...
function processFilters(filters)
{
  ...
  for (let filter of filters)
  {
    ...
    if (match)
    {
      ...
      filter.pattern = match[1] + filter.hostname + match[3];

      filter.justHostname = match[3].length < 2;
      filter.canMatchAsLowercase = filter.justHostname ||
                                   !/[a-zA-Z]/.test(match[3]);
    }
