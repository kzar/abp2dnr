...
// to be handled differently, so let's use a Symbol for this edge-case. That's
// cheaper and easier to compare than ["main_frame", "sub_frame"].
...
function generateCSPRules(filter, urlFilter, matchCase)
{
  ...
  return conditions.map(condition =>
  {
    if (filter instanceof AllowingFilter)
    {
      ...
      // the whole website. Unfortunately, I don't think that's possible if we
      // are to also support the distinction between specific and generic rules.
      //   Luckily, we are adding an "allow" rule (not "allowAllRequest") here
      // and there is no such thing as a blocking filter which applies to the
